---
title: "MALASPINA SRF | 18S amplicons"
...
#read data 
otu_tb18_5000 <- read.table(file="/home/laura/Documents/TFM/home/data/MALASPINA/Malaspina_18S_Surface/otus_18S_nohyphen.txt", head=TRUE, fill=TRUE)

#table dimensions and format before setting column names
#dim(otu_tb18_5000) # 56116   125
...
amplicons_per_sample_tb18<-colSums(otu_tb18_5000)
amplicons_per_sample_tb18[which(colSums(otu_tb18_5000)<5000)]
#the sample MD985 has less than 5000 OTUs 

#remove samples with less than 5000 OTUs in the MP_16S_ss5000 dataset (so that we can compare the relative abundance of 16S and 18S OTUs considering the same samples)
otu_tb18_min5000_v2<-subset(otu_tb18_min5000, select=-c(MD2304,MD2332,MD2695,MD2926))
```
...
min(colSums(otu_tb18_min5000_v2)) 
#8764
```
...
max(colSums(otu_tb18_min5000_v2)) 
# max: 1007852
```
...
amplicons_per_sample<-colSums(otu_tb18_min5000_v2)
amplicons_per_sample[which(colSums(otu_tb18_min5000_v2)>1000000)]
```

Overall reads per sample:

``` {r reads_per_sample_overview4, echo=FALSE}
plot(sort(colSums(otu_tb18_min5000_v2)), pch=19, xlab="sample", ylab="reads per sample", cex=0.9)
```
...
library(vegan)
otu_tb18_min5000_v2_t<-t(otu_tb18_min5000_v2)
otu_tb18_t_ss5000<-rrarefy(otu_tb18_min5000_v2_t, 5000)
```
...
There are 30925 OTUs that don't show any occurrence in the normalized data. Let's remove them from the table and take a look at its final dimensions:
...
#The final dimensions of the normalized table are 119 25093.
#25093+31023 = 56116
```

Datasets summary:

otu_tb18_min5000_v2_t --> 119 56116

otu_tb18_t_ss5000_no_cero --> 119 25191
...
otu_tb18_min5000_radfit<-radfit(colSums(otu_tb18_min5000_v2_t))
...
otu_tb18_min5000_prestonfit<-prestonfit(colSums(otu_tb18_min5000_v2_t))
plot(otu_tb18_min5000_prestonfit, main="Pooled species")

veiledspec(otu_tb18_min5000_prestonfit)
```

When computing Preston's lognormal model fit without pooling data into groups, we seem to miss ~3055 species:

```{r preston_model2, echo=4}
otu_tb18_min5000_dist_all<-prestondistr(colSums(otu_tb18_min5000_v2_t))
...
plot(geo_distances_MP_18S_ss5000, otu_tb18_t_ss5000_no_cero.bray, pch=19, cex=0.4, xlab="Geopgraphical distances", ylab="Bray-Curtis dissimilarities")
```
...
otu_tb18_t_ss5000_no_cero_t.rabund_means.occurence_perc<-as.data.frame(100*(rowSums(otu_tb18_t_ss5000_no_cero_t.rabund.occur)/119))
